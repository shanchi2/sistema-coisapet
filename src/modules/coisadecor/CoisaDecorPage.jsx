...
function OrderModal({ open, onClose, onSave, initial, initialItems, loading }) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="md"
      title={initial ? `Editar — ${initial.title}` : 'Novo pedido CoisaDecor'}
      footer={
        <>
...
// ── Página principal ──────────────────────────────────────────────
export function CoisaDecorPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [ordersRes, itemsRes] = await Promise.all([
      supabase.from('coisadecor_orders').select('*').order('created_at',{ascending:false}),
      supabase.from('coisadecor_items').select('*').order('created_at',{ascending:true}),
    ])
  ...
  async function handleSave(form, items) {
    ...
    try {
      ...
      if (editing) {
        const { error: updErr } = await supabase.from('coisadecor_orders').update(payload).eq('id',editing.id)
        if (updErr) throw new Error('Update: ' + updErr.message)
      } else {
        const { data, error: insErr } = await supabase.from('coisadecor_orders')
          .insert({...payload, created_by:uid}).select('id').single()
        if (insErr) throw new Error('Insert: ' + insErr.message)
        orderId = data.id
      }
      // Salva itens — deleta antigos e reinsere
      await supabase.from('coisadecor_items').delete().eq('order_id', orderId)
      if (validItems.length) {
        await supabase.from('coisadecor_items').insert(
          validItems.map(i => ({
            order_id:   orderId,
            name:       i.name.trim(),
  ...
  async function handleMove(order, newStatus) {
    ...
    await supabase.from('coisadecor_orders').update(upd).eq('id', order.id)
    load()
  }

  async function handleDelete(order) {
    await supabase.from('coisadecor_orders').delete().eq('id',order.id)
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C5904A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <p className="text-sm text-slate-400">Pedidos de produção personalizada</p>
        </div>
