...
function TaskModal({ task, users, open, onClose, onSave, onDelete, canSeeAtendimento = true, userRole = '', escritorioSector = null }) {
  ...
  const canEdit = isEscritorioRole
    ? false // escritório é sempre somente-leitura, não importa o setor da tarefa
    : isMarketplaceRole
  ...
  useEffect(() => {
    if (!task || selectedSector !== 'administrativo') return
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div>
          ...
          <select className="select" value={form.status} onChange={e=>set('status',e.target.value)} disabled={isEscritorioRole}>
        ...
        <div>
          ...
          <div className={`flex items-center gap-2 rounded-xl border px-2.5 ${prio.bg} ${prio.border}`}>
            ...
            <select className={`select !border-0 !bg-transparent !px-0 flex-1 font-bold ${prio.color}`}
              value={form.priority} onChange={e=>set('priority',e.target.value)} disabled={isEscritorioRole}>
        ...
        <div>
          ...
          <select className="select" value={form.assigned_to} onChange={e=>set('assigned_to',e.target.value)} disabled={isEscritorioRole}>
        ...
        <div>
          ...
          <select className="select" value={assignees[0] || ''} disabled={isEscritorioRole}
      ...
      <div className="grid grid-cols-[1fr_180px] gap-3">
        <div>
          ...
          <input className="input text-base font-semibold" value={form.title} readOnly={isEscritorioRole}
            onChange={e=>set('title',e.target.value)} placeholder="Descreva a tarefa..."/>
        </div>
        <div>
          <label className="form-label">Data limite</label>
          <input type="date" readOnly={isEscritorioRole} className={`input ${
      ...
      <div className="flex-1">
        ...
        <textarea className="textarea w-full" rows={10} readOnly={isEscritorioRole}
  ...
  const PanelSubtasks = (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {subtasks.map(sub => (
          <div key={sub.id}
            draggable={!isEscritorioRole}
            onDragStart={e => { if(isEscritorioRole) return; e.dataTransfer.effectAllowed='move'; setDragSub(sub.id) }}
            onDragOver={e => { if(isEscritorioRole) return; e.preventDefault(); e.dataTransfer.dropEffect='move' }}
            onDragEnter={e => { if(isEscritorioRole) return; e.preventDefault() }}
            onDrop={e => { if(isEscritorioRole) return; e.preventDefault(); reorderSubtask(dragSub, sub.id); setDragSub(null) }}
            onDragEnd={() => setDragSub(null)}
            className={`flex items-center gap-2 p-3 rounded-xl border transition-all
              ${isEscritorioRole ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'}
            ...
            {/* Handle drag */}
            {!isEscritorioRole && <span className="text-slate-300 hover:text-slate-400 cursor-grab shrink-0 select-none text-xs">⠿</span>}
            <button onClick={()=>!isEscritorioRole && toggleSubtask(sub)} disabled={isEscritorioRole}
              className={`shrink-0 transition-transform ${isEscritorioRole ? 'cursor-default' : 'hover:scale-110'}`}>
              {sub.done
      ...
      {/* Input nova subtask — escondido pro escritório (somente leitura) */}
      {!isEscritorioRole && (
    ...
    <Modal open={open} onClose={onClose} size="task"
      ...
      <div className="flex h-full" style={{minHeight:'560px'}}>
        ...
        <div className="w-[460px] shrink-0 pr-6 overflow-y-auto flex flex-col gap-4">
          <div>
            <label className="form-label">Título *</label>
            <input className="input text-lg font-bold" value={form.title} readOnly={isEscritorioRole}
              onChange={e=>set('title',e.target.value)} placeholder="Descreva a tarefa..."/>
          </div>
          <div className="flex-1 flex flex-col">
            <label className="form-label">Descrição</label>
            <textarea className="textarea w-full flex-1" rows={12} readOnly={isEscritorioRole}
        ...
        <div className="w-64 shrink-0 px-5 overflow-y-auto flex flex-col gap-4">
          ...
          <div>
            ...
            <select className="select text-sm" value={form.status} onChange={e=>set('status',e.target.value)} disabled={isEscritorioRole}>
              {COLUMNS.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            <select className="select text-sm" value={form.assigned_to} onChange={e=>set('assigned_to',e.target.value)} disabled={isEscritorioRole}>
              {assignableUsers.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            <select className="select text-sm" value={assignees[0] || ''} disabled={isEscritorioRole}
              onChange={e => setAssignees(e.target.value ? [e.target.value] : [])}>
              <option value="">Ninguém</option>
              {assignableUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          ...
          <div>
            ...
            <input type="date" readOnly={isEscritorioRole} className={`input text-sm ${
              dueStatus==='overdue' ? '!border-rose-300 !bg-rose-50 !text-rose-700 font-semibold' :
              dueStatus==='today'   ? '!border-sky-300 !bg-sky-50 !text-sky-700 font-semibold' : ''
            }`} value={form.due_date} onChange={e=>set('due_date',e.target.value)}/>
            {dueStatus==='overdue' && <p className="text-[10px] text-rose-500 font-semibold mt-1">⚠ Atrasada</p>}
...
export function KanbanOperacionalPage() {
  ...
  async function load(){
    ...
    setUsers((userR.data??[]).filter(u => !['equipe','horista','escritorio'].includes(u.role)))
    setLoading(false)
  ...
  async function saveTask(data, assigneeIds=[]){
    if(isEscritorioRole){ toast.error('Acesso somente leitura.'); return }
    const{id:uid}=getSession()
    ...
    <div className="flex flex-col gap-5 h-full animate-fade-in">
      <div className="page-header">
        ...
        <div className="flex items-center gap-2">
          ...
          {isEscritorioRole ? (
            <span className="flex items-center gap-1.5 text-xs font-bold text-slate-400 bg-slate-100 px-3 py-2 rounded-xl">
        ...
        <div className="grid gap-5 flex-1 pb-4" style={{gridTemplateColumns:`repeat(${COLUMNS.filter(c=>showDone||c.id!=='done').length},minmax(0,1fr))`,minHeight:0,overflow:'hidden'}}>
          {COLUMNS.filter(c=>showDone||c.id!=='done').map((col,idx)=>(
            <KanbanColumn key={col.id} col={col} colIndex={idx} tasks={tasksByCol[col.id]||[]} users={users}
              onOpen={openTask} onMove={moveTask} onDrop={dropTask} readOnly={isEscritorioRole}/>
          ))}
        </div>
      )}
