...
// 'today' até 23:59:59 do dia da data limite não conta como atrasado.
...
const TASK_TYPES = [
  ...
  { color: '#3b82f6', label: 'Colaborador'    },
  { color: '#22c55e', label: 'Site'           },
  { color: '#f59e0b', label: 'Marketing'      },
  { color: '#8b5cf6', label: 'Marketplace'    },
  { color: '#64748b', label: 'Operacional'    },
  { color: '#e85854', label: 'Infra-estrutura'},
  { color: '#ec4899', label: 'Parcerias'      },
  { color: '#06b6d4', label: 'Visual/Mídia'   },
  { color: '#004450', label: 'Embalagens'     },
  { color: '#00d60b', label: 'Análise & Admin' },
  { color: '#ff00ea', label: 'Sistema' },
]
const CARD_COLORS = TASK_TYPES.map(t => t.color)

// ─── TaskCard ─────────────────────────────────────────────────────
function TaskCard({ task, users, onOpen, onMove, colIndex }) {
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex-1 min-w-0">
            {task.task_code && (
              <span className="inline-block text-[9px] font-black font-mono tracking-widest text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded mb-1">
                #{task.task_code}
              </span>
            )}
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        style={value ? { background: value+'18', borderColor: value+'66', color: value, fontWeight: 700 } : {}}>
        {selected ? (
          <>
          ...
          <span className="flex-1 text-left text-slate-400">— Sem categoria</span>
        )}
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="max-h-56 overflow-y-auto">
            {TASK_TYPES.filter(t=>t.color).map(({color, label}) => (
              <button key={color} type="button"
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm font-semibold hover:opacity-80 transition-opacity
                  ${value === color ? 'opacity-100' : 'opacity-80'}`}
...
function TaskModal({ task, users, open, onClose, onSave, onDelete }) {
  const [form,    setForm]    = useState({title:'',description:'',status:'backlog',priority:'media',due_date:'',assigned_to:'',color:null})
  const [assignees,   setAssignees]   = useState([]) // IDs dos usuários co-responsáveis
  ...
  async function sendComment(){
    ...
    const link  = task.task_code ? `/kanban?task=${task.task_code}` : null
  ...
  const PanelDetails = (
    <div className="flex flex-col gap-4 h-full">
      ...
      {/* Linha 0 — Metadados somente-leitura */}
      {!isNew && (
      ...
      <div className="grid grid-cols-4 gap-3">
        {/* Categoria dropdown customizado com cores */}
  ...
  // ── Layout WIDE (≥1440×900) — 5 colunas: Título/Descrição | Subtasks | Arquivos | Informações | Comentários ──
      ...
      <div className="flex h-full" style={{minHeight:'560px'}}>
        ...
        <div className="w-64 shrink-0 px-5 overflow-y-auto flex flex-col gap-4">
          ...
          <div>
            ...
            <CategoryDropdown value={form.color ?? null} onChange={v => set('color', v)}/>
          </div>
...
// ─── Coluna ───────────────────────────────────────────────────────
function KanbanColumn({ col, tasks, users, onOpen, onMove, onDrop, colIndex }) {
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1" style={{minWidth:0}}>
      ...
      <div className={`flex flex-col gap-3 flex-1 p-2 rounded-2xl transition-all min-h-[120px] overflow-y-auto ${over?'bg-slate-100 ring-2 ring-slate-200':''}`}
        ...
        onDrop={e=>{e.preventDefault();setOver(false);const id=e.dataTransfer.getData('taskId');if(id)onDrop(id,col.id)}}>
        {tasks.length===0&&!over&&(
        ...
        {tasks.map(t=><TaskCard key={t.id} task={t} users={users} colIndex={colIndex} onOpen={onOpen} onMove={onMove}/>)}
      </div>
    </div>
  )
}
...
export function KanbanPage() {
  ...
  const [filter,    setFilter]    = useState({priority:'',assigned:'',search:'',color:''})
  ...
  async function load(){
    ...
    const [taskR,userR]=await Promise.all([
      supabase.from('tasks').select('*, task_code, comments:task_comments(count), attachments:task_attachments(count), subtasks:task_subtasks(count)').eq('kanban_type','diretoria').order('position',{ascending:true}).order('created_at',{ascending:false}),
  ...
  async function saveTask(data, assigneeIds=[]){
    const{id:uid}=getSession()
    const link = data.task_code ? `/kanban?task=${data.task_code}` : null
    if(data.id){
      const before = tasks.find(t=>t.id===data.id)
      const{error}=await supabase.from('tasks').update({title:data.title,description:data.description,status:data.status,priority:data.priority,due_date:data.due_date,assigned_to:data.assigned_to,color:data.color}).eq('id',data.id)
      ...
      if (recipients.length) {
        ...
        await createNotifications({
          taskId: data.id, taskCode: data.task_code, taskTitle: data.title,
          type: 'task_moved',
          body: changeDesc,
      ...
      const{data:newTask,error}=await supabase.from('tasks').insert({title:data.title,description:data.description,status:data.status,priority:data.priority,due_date:data.due_date,assigned_to:data.assigned_to,color:data.color,created_by:uid,position:maxPos+1,kanban_type:'diretoria'}).select('id, task_code').single()
      if(!error && newTask?.id) {
        ...
        const recipients = members.filter(id => id !== uid)
        if (recipients.length) {
          const newLink = newTask.task_code ? `/kanban?task=${newTask.task_code}` : null
          ...
          await createNotifications({
            ...
            type:         'task_created',
            body:         `Nova tarefa${data.due_date ? ` · Prazo: ${fmtDate(data.due_date)}` : ''}${responsavel ? ` · Responsável: ${responsavel}` : ''}`,
            recipientIds: recipients,
            link:         newLink,
          })
}
  ...
  return (
    <div className="flex flex-col gap-5 h-full animate-fade-in">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {(filter.search||filter.priority||filter.assigned||filter.color)&&(
          <button onClick={()=>setFilter({priority:'',assigned:'',search:'',color:''})} className="btn-secondary text-xs"><X size={12}/> Limpar</button>
        )}
      </div>
      ...
      <TaskModal task={selTask} users={users} open={modalOpen} onClose={()=>setModalOpen(false)} onSave={saveTask} onDelete={t=>setDelTask(t)}/>
