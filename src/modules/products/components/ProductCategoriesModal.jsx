  ...
  return (
    <div className="border border-slate-200 rounded-xl p-4 bg-slate-50 flex flex-col gap-3">
      <div>
        ...
        <input
          className="input"
          placeholder="Ex: Terrário, Acessório, Kit..."
          value={name}
...
export function ProductCategoriesModal({ open, onClose }) {
  const { categories, loading, create, update, remove } = useProductCategories()
  ...
  return (
    <>
      ...
      >
        {!showNew && (
          <button onClick={() => setShowNew(true)} className="btn-primary w-full mb-4">
            <Plus size={16} /> Nova categoria
          </button>
        )}
        {showNew && (
          <div className="mb-4">
            <CategoryForm onSave={handleCreate} onCancel={() => setShowNew(false)} loading={saving} />
          </div>
        )}
        {loading ? (
            ...
            {categories.map((cat) =>
              editingId === cat.id ? (
                <CategoryForm
                  key={cat.id}
                  initial={cat}
                  onSave={(p) => handleUpdate(cat.id, p)}
                ...
                <div key={cat.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-100 bg-white">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setEditingId(cat.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-sky-500 hover:bg-sky-50 transition-all">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => setDeleteTarget(cat)} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
      ...
      <ConfirmDialog
        ...
        description="Produtos vinculados a esta categoria não serão excluídos, apenas perderão a categoria."
