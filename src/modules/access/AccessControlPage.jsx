    ...
    <div className="p-6 max-w-5xl mx-auto">
        ...
        {ROLES.map(r => (
          <div key={r.key} className={`p-4 rounded-2xl border ${r.bg} flex items-center justify-between`}>
            ...
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${r.bg}`}>
              {r.key === 'administrativo' ? '👔' : r.key === 'atendimento' ? '🎧' : '⚙️'}
            </div>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          {/* Cabeçalho */}
          <div className="grid grid-cols-[1fr_110px_110px_110px] gap-0 border-b border-slate-100">
            ...
            return (
              <div key={section}>
                ...
                {sectionModules.map((mod, idx) => (
                  <div
                    key={mod.key}
                    className={`grid grid-cols-[1fr_110px_110px_110px] items-center gap-0
