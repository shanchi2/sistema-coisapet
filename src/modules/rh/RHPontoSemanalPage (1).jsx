import { useState, useEffect, useMemo } from 'react'
import { Clock, ChevronLeft, ChevronRight, FileDown, Calendar, Pencil, X, DollarSign, Check } from 'lucide-react'
import { supabase } from '../../lib/supabase'
import { fmtTime, fmtH, Avatar, PageHeader, LoadingCard, EmptyState } from './rhHelpers'
import toast from 'react-hot-toast'

const fmtBRL = v => (v ?? 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })

// Converte "HH:MM" + date para ISO string com offset do browser — mesmo
// helper usado no Relatório de Ponto, pra 08:00 digitado virar 08:00-03:00
function toISO(date, timeStr) {
  if (!timeStr) return null
  const d = new Date(`${date}T${timeStr}:00`)
  const off = d.getTimezoneOffset()
  const sign = off <= 0 ? '+' : '-'
  const h = String(Math.floor(Math.abs(off) / 60)).padStart(2, '0')
  const m = String(Math.abs(off) % 60).padStart(2, '0')
  return `${date}T${timeStr}:00${sign}${h}:${m}`
}

// ─── Helpers de semana (Segunda a Domingo) ──────────────────────────
function getMonday(dateStr) {
  const d = new Date(dateStr + 'T12:00:00')
  const day = d.getDay() // 0=domingo .. 6=sábado
  const diff = day === 0 ? -6 : 1 - day
  d.setDate(d.getDate() + diff)
  return d.toISOString().split('T')[0]
}
function addDays(dateStr, n) {
  const d = new Date(dateStr + 'T12:00:00')
  d.setDate(d.getDate() + n)
  return d.toISOString().split('T')[0]
}
function fmtDiaCurto(dateStr) {
  return new Date(dateStr + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: '2-digit' })
    .replace('.', '')
}
const DIAS_SEMANA = ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo']

// ─── Modal de edição do dia — mesmo padrão do Relatório de Ponto ────
function EditDayModal({ open, onClose, date, empId, records, onSaved }) {
  const PUNCHES = [
    { type: 'entrada',      label: 'Entrada',      color: 'text-emerald-600' },
    { type: 'saida_almoco', label: 'Saída Almoço', color: 'text-amber-500'   },
    { type: 'volta_almoco', label: 'Retorno',      color: 'text-blue-500'    },
    { type: 'saida',        label: 'Saída',        color: 'text-violet-600'  },
  ]

  const [times,  setTimes]  = useState({})
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    if (!open) return
    const init = {}
    PUNCHES.forEach(({ type }) => {
      const rec = records.find(r => r.punch_type === type)
      init[type] = rec ? fmtTime(rec.recorded_at) : ''
    })
    setTimes(init)
  }, [open, records])

  async function handleSave() {
    setSaving(true)
    try {
      for (const { type } of PUNCHES) {
        const existing = records.find(r => r.punch_type === type)
        const val = times[type]?.trim()

        if (val) {
          const iso = toISO(date, val)
          const originalTime = existing ? fmtTime(existing.recorded_at) : null
          const wasChanged   = val !== originalTime
          const isNew        = !existing

          if (wasChanged || isNew) {
            const { error } = await supabase.rpc('admin_upsert_punch', {
              p_employee_id: empId,
              p_punch_type:  type,
              p_date:        date,
              p_recorded_at: iso,
              p_record_id:   existing?.id ?? null,
            })
            if (error) throw error

            let targetId = existing?.id ?? null
            if (!targetId) {
              const { data: fresh } = await supabase
                .from('time_records')
                .select('id')
                .eq('employee_id', empId).eq('date', date).eq('punch_type', type)
                .single()
              targetId = fresh?.id ?? null
            }
            if (targetId) {
              await supabase.rpc('admin_mark_manual', { p_record_id: targetId })
            }
          }
        } else if (!val && existing) {
          const { error } = await supabase.rpc('admin_delete_punch', { p_record_id: existing.id })
          if (error) throw error
        }
      }

      // Recalcula hours_worked — mesma RPC do Relatório de Ponto
      await supabase.rpc('admin_recalc_hours', { p_employee_id: empId, p_date: date })

      toast.success('Ponto ajustado com sucesso!')
      onSaved()
      onClose()
    } catch (e) {
      toast.error('Erro ao salvar: ' + e.message)
    } finally {
      setSaving(false)
    }
  }

  if (!open) return null

  const dayLabel = new Date(date + 'T12:00:00').toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={onClose} />
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-100 p-6 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-slate-800 capitalize" style={{ fontFamily: 'Nunito,sans-serif' }}>{dayLabel}</h3>
            <p className="text-xs text-slate-400">Ajuste manual de ponto</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100"><X size={16} /></button>
        </div>

        <div className="flex flex-col gap-3">
          {PUNCHES.map(({ type, label, color }) => (
            <div key={type} className="flex items-center gap-3">
              <span className={`text-xs font-bold w-24 shrink-0 ${color}`}>{label}</span>
              <input type="time" className="input flex-1" value={times[type] || ''}
                onChange={e => setTimes(t => ({ ...t, [type]: e.target.value }))} />
            </div>
          ))}
        </div>

        <p className="text-[11px] text-slate-400">Deixar um campo vazio remove o registro daquele horário.</p>

        <button onClick={handleSave} disabled={saving} className="btn-primary justify-center disabled:opacity-50">
          {saving ? 'Salvando...' : 'Salvar ajuste'}
        </button>
      </div>
    </div>
  )
}


// ─── Reconstrói os "turnos" a partir das entradas ────────────────────
// Um turno começa numa 'entrada' e engloba tudo que vem depois (mesmo
// cruzando a meia-noite) até a próxima 'entrada'. O turno inteiro conta
// pro dia da entrada — é assim que turno noturno funciona aqui.
function groupIntoShifts(records) {
  const shifts = []
  let current = null
  records.forEach(r => {
    if (r.punch_type === 'entrada') {
      current = { day: r.date, records: [r] }
      shifts.push(current)
    } else if (current) {
      current.records.push(r)
    }
    // registro sem 'entrada' correspondente no período buscado é ignorado
    // (pertence a um turno que começou antes do período carregado)
  })
  return shifts
}

// ─── Resume um dia a partir do(s) turno(s) que começaram nele ────────
// Resume um único turno. Detecta o padrão "sem intervalo": bateu entrada +
// 'Saída Almoço' e nunca voltou (nem bateu 'Saída') — nesse caso o botão de
// almoço foi usado, na prática, como a saída final do turno.
function summarizeShift(shift) {
  const entrada  = shift.records.find(r => r.punch_type === 'entrada')
  const almoco_s = shift.records.find(r => r.punch_type === 'saida_almoco')
  const almoco_v = shift.records.find(r => r.punch_type === 'volta_almoco')
  const saidasReal = shift.records.filter(r => r.punch_type === 'saida')
  const saidaReal = saidasReal[saidasReal.length - 1] || null

  const semIntervalo = almoco_s && !almoco_v && !saidaReal

  if (semIntervalo) {
    const horas = entrada ? (new Date(almoco_s.recorded_at) - new Date(entrada.recorded_at)) / 3600000 : null
    return { entrada, almoco_s: null, almoco_v: null, saida: almoco_s, totalH: horas, incomplete: false, semIntervalo: true }
  }

  const totalH = saidaReal?.hours_worked ? parseFloat(saidaReal.hours_worked) : null
  return { entrada, almoco_s, almoco_v, saida: saidaReal, totalH, incomplete: !saidaReal, semIntervalo: false }
}

function summarizeDay(dayShifts) {
  if (!dayShifts || dayShifts.length === 0) {
    return { entrada: null, almoco_s: null, almoco_v: null, saida: null, totalH: null, incomplete: false, hasRecords: false, extraShifts: 0 }
  }
  const shiftSummaries = dayShifts.map(summarizeShift)
  const first = shiftSummaries[0]
  const last  = shiftSummaries[shiftSummaries.length - 1]

  const algumFechado = shiftSummaries.some(s => s.saida != null)
  const totalH = algumFechado
    ? shiftSummaries.reduce((sum, s) => sum + (s.totalH || 0), 0)
    : null

  const incomplete = shiftSummaries.some(s => s.incomplete)
  const { entrada, almoco_s, almoco_v } = first
  const { saida } = last
  // Saída cruzou pra o dia seguinte? (turno noturno)
  const crossedMidnight = saida && entrada && new Date(saida.recorded_at).toISOString().split('T')[0] !== entrada.date

  return { entrada, almoco_s, almoco_v, saida, totalH, incomplete, hasRecords: true, extraShifts: dayShifts.length - 1, crossedMidnight }
}

// ─── Geração do PDF (folha de ponto semanal) ─────────────────────────
async function generateWeeklyPDF(employee, weekStart, days) {
  const { default: jsPDF } = await import('jspdf')
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  const PAGE_W = 210, MARGIN = 14

  // Cabeçalho
  doc.setFillColor(61, 31, 13) // marrom escuro CoisaPet
  doc.rect(0, 0, PAGE_W, 26, 'F')
  doc.setTextColor(196, 149, 106) // gold
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(15)
  doc.text('CoisaPet', MARGIN, 12)
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text('Folha de Ponto Semanal', MARGIN, 19)

  const weekEnd = addDays(weekStart, 6)
  const periodo = `${new Date(weekStart+'T12:00:00').toLocaleDateString('pt-BR')} a ${new Date(weekEnd+'T12:00:00').toLocaleDateString('pt-BR')}`

  // Dados do funcionário
  let y = 36
  doc.setTextColor(61, 31, 13)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  doc.text(employee.name, MARGIN, y)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(10)
  doc.setTextColor(100, 100, 100)
  doc.text(`${employee.job_title || 'Horista'}  ·  Período: ${periodo}`, MARGIN, y + 6)

  // Tabela
  y += 16
  const colX = [MARGIN, MARGIN + 32, MARGIN + 72, MARGIN + 108, MARGIN + 144, MARGIN + 164]
  const colW = [32, 40, 36, 36, 20, 18]
  const rowH = 10
  const headers = ['Dia', 'Entrada', 'Saída Almoço', 'Volta Almoço', 'Saída', 'Total']

  doc.setFillColor(241, 226, 205) // marrom claro
  doc.rect(MARGIN, y, PAGE_W - MARGIN * 2, rowH, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(8)
  doc.setTextColor(110, 63, 37)
  headers.forEach((h, i) => doc.text(h, colX[i] + 2, y + 6.5))
  y += rowH

  let weekTotal = 0
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  days.forEach(({ date, label, summary }, idx) => {
    if (idx % 2 === 1) {
      doc.setFillColor(250, 248, 245)
      doc.rect(MARGIN, y, PAGE_W - MARGIN * 2, rowH, 'F')
    }
    doc.setTextColor(60, 60, 60)
    doc.text(`${label} ${new Date(date+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})}`, colX[0] + 2, y + 6.5)
    doc.text(summary.entrada  ? fmtTime(summary.entrada.recorded_at)  : '--:--', colX[1] + 2, y + 6.5)
    doc.text(summary.almoco_s ? fmtTime(summary.almoco_s.recorded_at) : '--:--', colX[2] + 2, y + 6.5)
    doc.text(summary.almoco_v ? fmtTime(summary.almoco_v.recorded_at) : '--:--', colX[3] + 2, y + 6.5)
    doc.text(summary.saida    ? fmtTime(summary.saida.recorded_at) + (summary.crossedMidnight ? ' (+1)' : '') : '--:--', colX[4] + 2, y + 6.5)
    const totalTxt = summary.totalH != null ? fmtH(summary.totalH) : (summary.incomplete ? '⚠' : '—')
    doc.text(totalTxt, colX[5] + 2, y + 6.5)
    if (summary.totalH) weekTotal += summary.totalH
    y += rowH
  })

  // Linha de total
  doc.setFillColor(61, 31, 13)
  doc.rect(MARGIN, y, PAGE_W - MARGIN * 2, rowH + 2, 'F')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.setTextColor(196, 149, 106)
  doc.text('TOTAL DA SEMANA', colX[0] + 2, y + 7.5)
  doc.setTextColor(255, 255, 255)
  doc.text(fmtH(weekTotal), colX[5] + 2, y + 7.5)
  y += rowH + 2

  // Linha de valor a pagar (só aparece se o funcionário tem valor/hora cadastrado)
  if (employee.hourly_rate > 0) {
    const valorPagar = weekTotal * employee.hourly_rate
    doc.setFillColor(241, 226, 205)
    doc.rect(MARGIN, y, PAGE_W - MARGIN * 2, rowH, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(110, 63, 37)
    doc.text(`Valor/hora: ${fmtBRL(employee.hourly_rate)}`, colX[0] + 2, y + 6.5)
    doc.setFontSize(10)
    doc.text(`Valor a pagar: ${fmtBRL(valorPagar)}`, colX[3] + 2, y + 6.5)
    y += rowH
  }
  y += 18

  // Assinaturas
  const sigW = 75
  doc.setDrawColor(180, 180, 180)
  doc.line(MARGIN, y, MARGIN + sigW, y)
  doc.line(PAGE_W - MARGIN - sigW, y, PAGE_W - MARGIN, y)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(120, 120, 120)
  doc.text('Assinatura do Funcionário', MARGIN, y + 5)
  doc.text('Assinatura do Responsável', PAGE_W - MARGIN - sigW, y + 5)

  // Rodapé
  doc.setFontSize(7)
  doc.setTextColor(180, 180, 180)
  doc.text(`Gerado em ${new Date().toLocaleString('pt-BR')} · CoisaPet Sistema`, MARGIN, 287)

  doc.save(`ponto_${employee.name.replace(/\s+/g,'_').toLowerCase()}_${weekStart}.pdf`)
}

// ─── Página principal ─────────────────────────────────────────────
export function RHPontoSemanalPage({ embedded = false } = {}) {
  const [employees, setEmployees] = useState([])
  const [records,   setRecords]   = useState([])
  const [loading,   setLoading]   = useState(true)
  const [selEmp,    setSelEmp]    = useState('')
  const [weekStart, setWeekStart] = useState(() => getMonday(new Date().toISOString().split('T')[0]))
  const [generating,setGenerating]= useState(false)
  const [editModal, setEditModal] = useState(null) // { date, records } ou null
  const [editingRate, setEditingRate] = useState(false)
  const [rateVal,     setRateVal]     = useState('')

  useEffect(() => { loadEmployees() }, [])
  useEffect(() => { if (selEmp) loadRecords() }, [selEmp, weekStart])

  async function loadEmployees() {
    const { data } = await supabase.from('system_users')
      .select('id,name,role,job_title,work_start,work_end,lunch_minutes,hourly_rate')
      .eq('active', true).eq('role', 'horista').order('name')
    setEmployees(data ?? [])
    if (data?.length === 1) setSelEmp(data[0].id) // se só tem 1 horista, já seleciona
  }

  async function loadRecords() {
    setLoading(true)
    const weekEnd = addDays(weekStart, 6)
    // Busca até 1 dia depois do fim da semana — pega a saída de um turno
    // que começou no domingo à noite e virou a madrugada de segunda.
    const { data } = await supabase.from('time_records')
      .select('*')
      .eq('employee_id', selEmp)
      .gte('date', weekStart).lte('date', addDays(weekEnd, 1))
      .order('recorded_at', { ascending: true })
    setRecords(data ?? [])
    setLoading(false)
  }

  const employee = employees.find(e => e.id === selEmp)

  async function saveHourlyRate() {
    const v = parseFloat(rateVal.replace(',', '.'))
    if (isNaN(v) || v < 0) { toast.error('Valor inválido'); return }
    const { error } = await supabase.from('system_users').update({ hourly_rate: v }).eq('id', selEmp)
    if (error) { toast.error('Erro ao salvar valor/hora'); return }
    setEmployees(prev => prev.map(e => e.id === selEmp ? { ...e, hourly_rate: v } : e))
    setEditingRate(false)
    toast.success('Valor/hora atualizado!')
  }

  const days = useMemo(() => {
    const shifts = groupIntoShifts(records)
    return Array.from({ length: 7 }, (_, i) => {
      const date = addDays(weekStart, i)
      const dayShifts = shifts.filter(s => s.day === date)
      return { date, label: DIAS_SEMANA[i], summary: summarizeDay(dayShifts) }
    })
  }, [weekStart, records])

  const weekTotal = days.reduce((acc, d) => acc + (d.summary.totalH || 0), 0)
  const weekEnd = addDays(weekStart, 6)

  async function handleGeneratePDF() {
    if (!employee) return
    setGenerating(true)
    try {
      await generateWeeklyPDF(employee, weekStart, days)
    } finally {
      setGenerating(false)
    }
  }

  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      {!embedded && (
        <PageHeader
          title="Ponto Semanal — Horistas"
          subtitle="Folha de ponto semanal, de segunda a domingo"
        />
      )}

      {employees.length === 0 ? (
        <EmptyState icon={Clock} title="Nenhum horista cadastrado" description="Não há usuários ativos com o cargo Horista." />
      ) : (
        <>
          {/* Filtros */}
          <div className="flex flex-wrap gap-3 items-center">
            <select className="select w-auto min-w-[220px]" value={selEmp} onChange={e => setSelEmp(e.target.value)}>
              <option value="">Selecione um horista...</option>
              {employees.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
            </select>

            {selEmp && (
              <div className="flex items-center gap-2">
                <button onClick={() => setWeekStart(w => addDays(w, -7))} className="btn-secondary text-xs px-3 py-2">
                  <ChevronLeft size={14} />
                </button>
                <span className="text-sm font-semibold text-slate-600 px-2 flex items-center gap-1.5">
                  <Calendar size={13} className="text-slate-400" />
                  {new Date(weekStart+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})} a {new Date(weekEnd+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric'})}
                </span>
                <button onClick={() => setWeekStart(w => addDays(w, 7))} className="btn-secondary text-xs px-3 py-2">
                  <ChevronRight size={14} />
                </button>
                <button onClick={() => setWeekStart(getMonday(new Date().toISOString().split('T')[0]))} className="btn-secondary text-xs px-3 py-2">
                  Semana atual
                </button>
              </div>
            )}

            {selEmp && (
              <button onClick={handleGeneratePDF} disabled={generating}
                className="btn-primary flex items-center gap-1.5 text-xs ml-auto">
                {generating
                  ? <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Gerando...</>
                  : <><FileDown size={14}/> Gerar PDF da semana</>
                }
              </button>
            )}
          </div>

          {!selEmp ? (
            <EmptyState icon={Clock} title="Selecione um horista" description="Escolha um funcionário horista para ver a folha de ponto da semana." />
          ) : loading ? (
            <LoadingCard />
          ) : (
            <div className="card overflow-hidden">
              {/* Cabeçalho do funcionário */}
              <div className="flex items-center gap-4 pb-4 mb-4 border-b border-slate-100 flex-wrap">
                <Avatar name={employee.name} size="lg" />
                <div>
                  <p className="font-bold text-slate-800">{employee.name}</p>
                  <p className="text-xs text-slate-400">{employee.job_title || 'Horista'}</p>
                </div>

                {/* Valor/hora — clicável pra editar */}
                <div className="flex items-center gap-1.5">
                  {editingRate ? (
                    <>
                      <DollarSign size={13} className="text-slate-400"/>
                      <input autoFocus type="text" inputMode="decimal" className="input w-24 text-sm py-1"
                        placeholder="0,00" value={rateVal} onChange={e=>setRateVal(e.target.value)}
                        onKeyDown={e=>{ if(e.key==='Enter') saveHourlyRate(); if(e.key==='Escape') setEditingRate(false) }}/>
                      <button onClick={saveHourlyRate} className="p-1.5 text-emerald-500 hover:bg-emerald-50 rounded-lg"><Check size={14}/></button>
                      <button onClick={()=>setEditingRate(false)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded-lg"><X size={14}/></button>
                    </>
                  ) : (
                    <button onClick={()=>{ setRateVal(String(employee.hourly_rate ?? '')); setEditingRate(true) }}
                      className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 px-2.5 py-1.5 rounded-xl transition-colors">
                      <DollarSign size={13}/>
                      {employee.hourly_rate > 0 ? `${fmtBRL(employee.hourly_rate)}/hora` : 'Definir valor/hora'}
                      <Pencil size={11} className="text-slate-300"/>
                    </button>
                  )}
                </div>

                <div className="ml-auto flex items-center gap-3">
                  {employee.hourly_rate > 0 && (
                    <div className="text-center px-5 py-2.5 rounded-2xl bg-amber-50 border border-amber-100">
                      <p className="text-[10px] text-amber-600 font-bold uppercase tracking-wide">Valor a pagar</p>
                      <p className="text-xl font-black text-amber-700" style={{fontFamily:'Nunito,sans-serif'}}>{fmtBRL(weekTotal * employee.hourly_rate)}</p>
                    </div>
                  )}
                  <div className="text-center px-5 py-2.5 rounded-2xl bg-emerald-50 border border-emerald-100">
                    <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-wide">Total da semana</p>
                    <p className="text-xl font-black text-emerald-700" style={{fontFamily:'Nunito,sans-serif'}}>{fmtH(weekTotal)}</p>
                  </div>
                </div>
              </div>

              {/* Tabela dos 7 dias */}
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-[10px] text-slate-400 font-bold uppercase tracking-wide">
                      <th className="pb-2 pr-3">Dia</th>
                      <th className="pb-2 px-3">Entrada</th>
                      <th className="pb-2 px-3">Saída Almoço</th>
                      <th className="pb-2 px-3">Volta Almoço</th>
                      <th className="pb-2 px-3">Saída</th>
                      <th className="pb-2 pl-3 text-right">Total</th>
                      <th className="pb-2 pl-3 w-10"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {days.map(({ date, label, summary }) => {
                      const isToday = date === new Date().toISOString().split('T')[0]
                      return (
                        <tr key={date} className={isToday ? 'bg-rose-50/40' : ''}>
                          <td className="py-2.5 pr-3">
                            <p className="font-semibold text-slate-700">{label}</p>
                            <p className="text-[10px] text-slate-400">{new Date(date+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})}</p>
                          </td>
                          <td className="py-2.5 px-3 font-mono text-slate-600">{summary.entrada  ? fmtTime(summary.entrada.recorded_at)  : <span className="text-slate-300">--:--</span>}</td>
                          <td className="py-2.5 px-3 font-mono text-slate-600">{summary.almoco_s ? fmtTime(summary.almoco_s.recorded_at) : <span className="text-slate-300">--:--</span>}</td>
                          <td className="py-2.5 px-3 font-mono text-slate-600">{summary.almoco_v ? fmtTime(summary.almoco_v.recorded_at) : <span className="text-slate-300">--:--</span>}</td>
                          <td className="py-2.5 px-3 font-mono text-slate-600">
                            {summary.saida
                              ? <>{fmtTime(summary.saida.recorded_at)}{summary.crossedMidnight && <span className="text-[10px] text-violet-500 font-sans font-bold ml-1">+1</span>}</>
                              : <span className="text-slate-300">--:--</span>}
                          </td>
                          <td className="py-2.5 pl-3 text-right font-bold">
                            {summary.totalH != null
                              ? <span className="text-slate-700">{fmtH(summary.totalH)}</span>
                              : summary.incomplete
                                ? <span className="text-amber-500 text-xs">⚠ em aberto</span>
                                : <span className="text-slate-300">—</span>
                            }
                          </td>
                          <td className="py-2.5 pl-3 text-right">
                            <button onClick={() => setEditModal({ date, records: records.filter(r => r.date === date) })}
                              className="p-1.5 rounded-lg text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-colors">
                              <Pencil size={14} />
                            </button>
                          </td>
                        </tr>
                      )
                    })}
                  </tbody>
                </table>
              </div>
            </div>
          )}
        </>
      )}

      {editModal && (
        <EditDayModal
          open={!!editModal}
          onClose={() => setEditModal(null)}
          date={editModal.date}
          empId={selEmp}
          records={editModal.records}
          onSaved={loadRecords}
        />
      )}
    </div>
  )
}
